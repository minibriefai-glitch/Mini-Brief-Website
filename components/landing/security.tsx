    ...
    <Section id={security.id}>
      ...
      {security.groups.map((group, g) => (
        <div key={group.title} className="mt-14">
          <h3 className="text-xs font-medium uppercase tracking-wider text-brand-muted-text">{group.title}</h3>
          <ul role="list" className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {group.items.map((item, i) => {
              ...
              return (
                <li key={item.title} className="rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-card">
                  ...
                  <h4 className="mt-5 text-lg font-semibold leading-snug text-brand-ink">{item.title}</h4>
      ...
      <div className="mt-10">
        <h3 className="text-xs font-medium uppercase tracking-wider text-brand-muted-text">{security.alsoLead}</h3>
